...
FFI_PLUGIN_EXPORT void arcfour_key_setup(BYTE state[], const BYTE key[], int len);

// Pseudo-Random Generator Algorithm
// Input: state - the state used to generate the keystream
//        out - Must be allocated to be of at least "len" length
//        len - number of bytes to generate
FFI_PLUGIN_EXPORT void arcfour_generate_stream(BYTE state[], BYTE out[], size_t len);
